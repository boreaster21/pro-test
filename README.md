...
2.  **(ホストマシン) 環境変数の設定:**
    `src/.env.example` をコピーして `src/.env` を作成します。
    ```bash
...
4.  **(ホストマシン) Docker コンテナのビルドと起動:**
    ...
    ```
    MySQL データは Docker 名前付きボリューム (`mysql_data`) に永続化されます。
...
-   **店舗検索・ソート:** 店舗一覧ページのヘッダーにあるフォームで、地域、ジャンル、キーワードでの絞り込み、および評価順/ランダムでの並び替えが可能です。
-   **お気に入り:** 店舗カードや詳細ページでお気に入り登録/解除が可能です。（要ログイン）
...
-   **CSVインポート:** ログイン後、ヘッダーのユーザー名ドロップダウンメニューから「CSVインポート」を選択し、`/admin/import/csv` ページにアクセスします。
...
このプロジェクトでは、アプリケーションの品質を保証するために PHPUnit を使用した Feature テストが実装されています。主要な機能である口コミ、店舗一覧ソート、CSVインポートに関するテストが含まれています。
